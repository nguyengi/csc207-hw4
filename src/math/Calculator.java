...
public class Calculator {
	...
	public Fraction evaluate(String expression) throws Exception {
			...
			try {
				...
				if (vals[i].charAt(0) == 'r'
						&& (Character.getNumericValue(vals[i].charAt(1)) > 7
								|| Character.getNumericValue(vals[i].charAt(1)) < 0 || vals[i]
								.length() != 2)) {
					...
					try {
						nums.add(new Fraction(new BigInteger(vals[i].substring(
								0, slash)), new BigInteger(vals[i].substring(
								slash + 1, vals[i].length()))));
		...
		while ((index = ops.lastIndexOf("^")) != -1) {
			Fraction temp = nums.get(index).expt(
					nums.get(index + 1).num.intValue());
		...
		while ((index3 = ops.lastIndexOf("=")) != -1) {
			...
			rs[Character.getNumericValue(vals[index3 * 2].charAt(1))] = nums
					.get(index3 + 1);
